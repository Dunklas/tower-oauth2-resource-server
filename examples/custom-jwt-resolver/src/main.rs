...
use tower_oauth2_resource_server::{
    claims::DefaultClaims, error::AuthError, jwt_resolver::BearerTokenResolver,
    jwt_unverified::UnverifiedJwt, server::OAuth2ResourceServer, tenant::TenantConfiguration,
};

struct CustomJwtResolver;

impl BearerTokenResolver for CustomJwtResolver {
    fn resolve(&self, request: &axum::http::Request<()>) -> Result<UnverifiedJwt, AuthError> {
...
async fn main() {
    ...
    let oauth2_resource_server = <OAuth2ResourceServer>::builder()
        ...
        .bearer_token_resolver(Arc::new(CustomJwtResolver))
        .build()
        .await
